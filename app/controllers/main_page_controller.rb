class MainPageController < ApplicationController
  def index
    ...
    @top_books = book_results[1..[10, book_results.length].min]
    movies = Work.where(category: "movie")
    movie_results = movies.sort_by { |work| -1*work.upvotes.count }
    @top_movies = movie_results[1..[10, movie_results.length].min]
    albums = Work.where(category: "album")
    album_results = albums.sort_by { |work| -1*work.upvotes.count }
    @top_albums = album_results[1..[10,album_results.length].min]
  end
end
